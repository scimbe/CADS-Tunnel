...
- **Publish the spec** — `PipelineSpec { id, roles: [RequiredRole{service, units}] }` is the "needed agent profiles + network info" a designer publishes (discoverable like a #144 directory entry / a topology).
- **Discover — two modes:**
  - **pull** ✅-ish: agents scan `/registry/agents` (#144) + the published spec + `CapacityOffer`s (#147/#167) and connect.
  - **push (invite by card) ⏳:** the pipeline invites specific agents using their **`AgentCard`** (#144: `holder_pubkey` + `card_url` + `role_tags`/`skill_ids`) — match each `RequiredRole` to the cards advertising it, then invite over a channel (`SignedChannelGrant`/#72 channel-invite). **Follow primitive:** `PipelineSpec::invitations(cards) → per-role candidate holders` (role↔`role_tags`/`skill_ids` match).
