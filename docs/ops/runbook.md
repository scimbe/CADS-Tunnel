...
| `CT_PORTAL_SESSION_KEY` | control plane | **HMAC key for the portal session cookie** (#294) — a *distinct* secret from the webhook one above, since that one is shared with an external payment provider; unset ⇒ a random key is generated at boot, so portal sessions just don't survive a restart until it's set |
| `CT_ADMIN_SUPER_EMAIL` | control plane | ADR-0025 — the one Google account allowed to reach the admin console and manage other admins. **Required, fail-closed**: the process refuses to start without it, same posture as `CT_EDGE_ADMIN_TOKEN` — unlike most vars in this table, there is no "unset ⇒ feature off" option here |
...
| `CT_BRIDGE_HOLDER_KEY` / `CT_BRIDGE_NOISE_KEY` | control plane | Agent-bridges-v2 — this deployment's own shared identity for dialing a tunnel owner's agent (over the platform's own channel broker) on their behalf, from the portal's "Agent bridges" page. 64 hex chars each (`openssl rand -hex 32` twice). **Optional, graceful** — unlike `CT_ADMIN_SUPER_EMAIL` above, unset or malformed just disables the dialer (the two Agent-bridges portal routes 503 clearly); it does **not** stop the process from booting |
| `CT_CHANNEL_BROKER` | control plane | Agent-bridges-v2 — `host:port` of the Agent-Fabric channel broker the bridge dialer above connects through; unset ⇒ defaults to this deployment's own production broker address (`bunsenbrenner.org:4435`) — not a secret, override only for a different/self-hosted broker |
